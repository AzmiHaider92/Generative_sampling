...
This version is modified to support *bidirectional* shortcut models:
the bootstrap dt can be positive or negative, and t is sampled so that
t and t + dt/2 stay inside [0, 1].
"""
...
def get_targets(cfg, gen, images, labels, call_model_fn, step,
    ...
    PyTorch port of get_targets (JAX). Returns:
      x_t, v_t, t, dt, labels_dropped, info

    Here:
      - For the bootstrap part, dt can be positive or negative (bidirectional).
      - For the flow-matching part, dt is positive and fixed.
    """
    ...
    if cfg.model_cfg.bootstrap_dt_bias == 0:
        ...
        levels = torch.arange(
            log2_sections - 1, -1, -1, device=device, dtype=torch.long
        )
        ...
        if dt_base.numel() < bootstrap_batchsize:
            pad = torch.zeros(
                bootstrap_batchsize - dt_base.numel(),
                device=device,
                dtype=torch.long,
            )
        ...
        short_levels = torch.arange(
            log2_sections - 3, -1, -1, device=device, dtype=torch.long
        )  # log2_sections-2 terms
        ...
        dt_base = torch.cat(
            [
                ...
                torch.zeros(part, device=device, dtype=torch.long),
            ],
            dim=0,
        )
        if dt_base.numel() < bootstrap_batchsize:
    ...
    # Positive magnitude of dt and its half-step:
    #   |dt| = 1 / 2^(dt_base), |dt_bootstrap| = |dt| / 2
    dt_mag = 1.0 / (2.0 ** dt_base.to(torch.float32))          # (B_b,)
    dt_bootstrap_mag = dt_mag / 2.0                            # (B_b,)

    # ---------- 1b) Make dt bidirectional (sign ∈ {-1, +1}) ----------
    sign = torch.randint(
        low=0,
        high=2,
    ...
    sign = sign * 2 - 1  # 0 -> -1, 1 -> +1
    sign = sign.to(torch.float32)

    dt = dt_mag * sign                     # signed dt (full interval, if you want it)
    dt_bootstrap = dt_bootstrap_mag * sign # signed half-step used for bootstrapping

    # ---------- 2) Sample t (bootstrap, aware of sign) ----------
    # We must ensure t and t + dt_bootstrap are in [0, 1].
    # For each example:
    #   if sign > 0:  t ∈ [0, 1 - |dt_bootstrap|]
    #   if sign < 0:  t ∈ [|dt_bootstrap|, 1]
    if bootstrap_batchsize > 0:
        u = torch.rand(bootstrap_batchsize, device=device, generator=gen)
        width = 1.0 - dt_bootstrap_mag  # > 0
        lo = torch.where(
            sign > 0.0,
        ...
        t = torch.empty(0, device=device, dtype=torch.float32)
    ...
    x_0 = torch.randn(
        x_1.shape, dtype=x_1.dtype, device=x_1.device, generator=gen
    )

    x_t = (1.0 - (1.0 - EPS) * t_full) * x_0 + t_full * x_1
    bst_labels = labels[:bootstrap_batchsize]

    # NOTE: dt_bootstrap is now signed and is what we pass into the model
    if not cfg.model_cfg.bootstrap_cfg:
        ...
        dt_extra = torch.cat(
            [dt_bootstrap, dt_bootstrap[:num_dt_cfg]], dim=0
        )
        ...
        v_b1_raw = call_model_fn(
            x_t_extra, t_extra, dt_extra, labels_extra, train=False
        )
        v_b1_cond = v_b1_raw[:x_1.shape[0]]
        v_b1_uncond = v_b1_raw[x_1.shape[0]:]
        v_cfg = v_b1_uncond + cfg_scale * (
            v_b1_cond[:num_dt_cfg] - v_b1_uncond
        )
        ...
        v_b2_raw = call_model_fn(
            x_t2_extra, t2_extra, dt_extra, labels_extra, train=False
        )
        v_b2_cond = v_b2_raw[:x_1.shape[0]]
        v_b2_uncond = v_b2_raw[x_1.shape[0]:]
        v_b2_cfg = v_b2_uncond + cfg_scale * (
            v_b2_cond[:num_dt_cfg] - v_b2_uncond
        )
    ...
    bst_v = v_target
    bst_dt = dt.clone()        # signed dt label for the bootstrap samples
    bst_t = t.clone()
    bst_xt = x_t
    bst_l = bst_labels
    ...
    drop = torch.bernoulli(
        torch.full((labels.shape[0],), class_dropout_prob, device=device)
    ).bool()
    ...
    t_flow = torch.randint(
        ...
        device=device,
        generator=gen,
    ).to(torch.float32) / float(denoise_timesteps)
    ...
    x1_flow = images
    x0_flow = torch.randn(
        x1_flow.shape, dtype=x1_flow.dtype, device=x1_flow.device, generator=gen
    )

    x_t_flow = (1.0 - (1.0 - EPS) * t_flow_full) * x0_flow + t_flow_full * x1_flow
    v_t_flow = x1_flow - (1.0 - EPS) * x0_flow

    # For flow matching we keep dt positive and fixed (no sign)
    dt_flow = torch.full(
        (B,),
        1.0 / denoise_timesteps,
        device=device,
        dtype=torch.float32,
    )
    ...
    dt_out = torch.cat([bst_dt, dt_flow[:bst_size_data]], dim=0)  # float32, signed for bootstrap
