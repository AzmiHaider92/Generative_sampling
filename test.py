...
def inference(
    ...
    Assumes model forward signature: m(x_bhwc, t, dt, y, train=False, return_activations=True)
    and that sampling produces BHWC in [-1,1] before optional VAE decode.
    """
    ...
    # -------- Helpers --------
    def call_model(x_bhwc, t_vec, dt_base, labels):
        return ema_model(x_bhwc, t_vec, dt_base, labels, train=False, return_activations=True)

    dt_flow = int(math.log2(cfg.model_cfg.denoise_timesteps))
    denoise_T = cfg.runtime_cfg.inference_timesteps
    delta_t   = 1.0 / denoise_T
    cfg_scale = cfg.runtime_cfg.inference_cfg_scale
    ...
    for _ in pbar:
        ...
        for ti in range(denoise_T):
            ...
            if cfg_scale == 0:
                v = call_model(x, t_vec, dt_base, labels_uncond)
            elif cfg_scale == 1:
                v = call_model(x, t_vec, dt_base, labels)
            else:
                v_un = call_model(x, t_vec, dt_base, labels_uncond)
                v_co = call_model(x, t_vec, dt_base, labels)
        ...
        if cfg.model_cfg.use_stable_vae:
            with torch.inference_mode(), torch.amp.autocast('cuda', torch.float16):
                x_vis = vae.decode(x1)                    # BHWC in [-1,1]
        x1 = (x_vis.clamp(-1, 1) + 1.0) * 0.5        # BHWC in [0,1]
