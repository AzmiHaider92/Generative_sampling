...
from models.meta_model import NP_DiT
...
from utils.np_utils import sample_ctx_tgt
...
def main():
    ...
    train_iter = get_dataset_iter(runtime_cfg.dataset_name, runtime_cfg.dataset_root_dir, per_rank_bs, True, runtime_cfg.debug_overfit, runtime_cfg.img_size)
    valid_iter = get_dataset_iter(runtime_cfg.dataset_name, runtime_cfg.dataset_root_dir, per_rank_bs, False, runtime_cfg.debug_overfit, runtime_cfg.img_size)
    ...
    def import_targets(train_type: str):
        name = {
            ...
            "shortcut_raw": "papers.shortcut_raw",
            "shortcut_cont": "papers.shortcut_cont",
            "shortcut_bi": "papers.shortcut_bidirectional",
    ...
    B, C, H = example_images.shape[0], example_images.shape[1], example_images.shape[-1]
    ...
    pixel_idx = torch.arange(runtime_cfg.img_size**2, device=device)  # Used to create coordinates for images

    while step < max_steps:
        t0 = time.time()

        img, _ = next(train_iter)
        img = maybe_encode(img).reshape((B, C, -1)).transpose(1, 2)  # (B, N, C)
        ...
        tgt_y_t, v_t, t_vec, dt_vec = get_targets(cfg, gen, ctx_tgt_xy, call_model, step)

        # forward + loss
        opt.zero_grad(set_to_none=True)
        with autocast('cuda', dtype=amp_dtype):
            v_pred = (dit)(ctx_tgt_xy['tgt_y'], ctx_tgt_xy, t_vec, dt_vec)
            mse = (v_pred - v_t).float().pow(2)
            loss = mse.mean()
